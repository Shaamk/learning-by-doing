...
def movies_list():
    ...
    for title, year in the_title_and_year_dict.items():
        print(f'{title}, {year}')
    return the_title_and_year_dict
...
def actor_list():
    ...
    # Actor Search : Prompt for an actor's name, then list the NAME OF MOVIES and YEAR OF MOVIES.
def actor_search(the_title_and_actor_dict, the_title_and_year_dict):
    actor_name = input(f'What is the name of the actor')
    for title, actor in the_title_and_actor_dict.items():
        for title, year in the_title_and_year_dict.items():
            if actor in the_title_and_actor_dict.items() and title in the_title_and_year_dict.items() == actorX:
