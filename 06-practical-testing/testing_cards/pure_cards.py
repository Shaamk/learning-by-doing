def parse_card(user_input):
    checked_invalid = check_for_invalid(user_input)
    if checked_invalid is not None:
        return checked_invalid
    else:
...
def get_card_description(user_input):
    card_dict = {}
    descriptions = {'A':'ace', '2':'two', '3':'three', '4':'four', '5':'five', '6':'six', '7':'seven', '8':'eight', '9':'nine', '10':'ten', 'J':'jack', 'Q':'queen', 'K':'king', 'C':'clubs', 'D':'diamonds', 'H':'hearts', 'S':'spades'}
...
def get_description(card_dict, rank, suit, description):
    if rank == 'A' or rank == '8':
        card_dict['description'] = f'an {description[rank]} of {description[suit]}'
    else:
        card_dict['description'] = f'a {description[rank]} of {description[suit]}'
    return card_dict
